...
import {
  AdminformatSessionStatus,
  AdmingetSessionStatusColor,
          ...
          <tbody>
            <AnimatePresence>
              {sessions.map((session, index) => {
                ...
                const statusLabel = AdminformatSessionStatus(session.status);
                const statusColor = AdmingetSessionStatusColor(session.status);
                  ...
                  >
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center space-x-1">
                        ...
                        {/* Cancel Button - Only show for scheduled sessions */}
                        {!cannotModify && onCancel && (
                          <motion.button
                            onClick={() => onCancel(session)}
                            whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                            className="bg-orange-600 hover:bg-orange-700 text-white p-2 rounded-lg transition-colors duration-200 shadow-sm cursor-pointer"
                        ...
                        {cannotModify && (
                          <motion.button
                            disabled={true}
                            className="bg-gray-400 text-white p-2 rounded-lg transition-colors duration-200 shadow-sm cursor-not-allowed"
                            title={
                              isCancelled
